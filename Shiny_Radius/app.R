...
server <- function(input, output, session) {
  ...
  dataRangeInput<-reactive({
    ...
    Surveillance <- Surveillance[Surveillance$DISPLAY_BRAND_NAME == input$Brand_P1,]
    Surveillance <- subset(Surveillance, Date >= input$DateFilter_P1[1] & Date <= input$DateFilter_P1[2]) 
    ...
    dataset <- dataset %>% top_n(5,dataset$Count)
    # write.csv(dataset,file="C:\\Users\\698190\\Documents\\Shiny_Radius\\Shiny_Radius\\Datasets\\output_Customer_Type.csv",row.names = F)
    # print(dataset)
    return(dataset)
  })
  ...
  output$barplot <-renderPlot({
    data1 <- dataRangeInput()
    ggplot(data= data1, aes_string(x=data1$CUSTOMER_TYPE_DESCRIPTION,y=data1$Count))  +
